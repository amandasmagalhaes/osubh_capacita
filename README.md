# OSUBH Capacita

Este repositório reúne os materiais desenvolvidos para os encontros do Grupo de Estudos e Capacitação do [Observatório de Saúde Urbana de Belo Horizonte (OSUBH)](https://osubh.medicina.ufmg.br).

A versão online está disponível em: <https://amandasmagalhaes.github.io/osubh_capacita/>


## Estrutura

Atualmente, o material está organizado da seguinte forma:

| Arquivo | Descrição |
|---------|-----------|
| `index.qmd` | Apresentação. |
| `01 - R.qmd` | Introdução ao software R. |
| `02 - Positron.qmd` | Introdução ao software Positron. |
| `03 - Introdução em R.qmd` | Primeiros passos na linguagem R. |
| `04 - GitHub.qmd` | Introdução ao Git e GitHub. |
| `05 - Boas práticas de programação.qmd` | Recomendações para organização e escrita de código. |
| `06 - Quarto.qmd` | Introdução à criação de documentos com Quarto. |
| `07 - Análise de temperatura e mortalidade.qmd` | Análise da associação entre temperatura e mortalidade utilizando DLNM. |
| `08 - GitHub Pages.qmd` | Introdução à publicação de projetos Quarto no GitHub Pages. |

Novos capítulos serão adicionados conforme a evolução dos encontros.